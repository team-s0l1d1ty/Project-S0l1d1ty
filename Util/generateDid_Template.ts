...
import { generateAccount, getAccount } from './generateAccount'
import * as envConfig from 'dotenv'
import { writeEnvVar, appendEnvVar} from "./genDotEnv";

export async function createFullDid(): Promise<Kilt.Did.FullDidDetails> {
  await Kilt.init({ address: process.env.WSS_ADDRESS })
  const { api } = await Kilt.connect()
  const mnemonic = process.env.XXX_MNEMONIC as string
...
generateAccount()
   ...
   .then(({ mnemonic, account }) => {
     ...
     envvar += `XXX_MNEMONIC="${mnemonic}"\n`
     envvar += `XXX_ADDRESS=${account.address}\n`
     writeEnvVar(envvar) 
     ...
     setTimeout(() => {
      createFullDid().catch((e)=>{
       ...
       appendEnvVar(`XXX_DID_URI=${did.uri}\n`)
       process.exit(1)
      })
     }, 60000);
    })
